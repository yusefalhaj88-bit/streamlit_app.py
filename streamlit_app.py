...
# روابط الصور المباشرة من حسابك (yusefalhaj88-bit)
BG_URL = "https://raw.githubusercontent.com/yusefalhaj88-bit/Python/main/background.png"
LOGO_URL = "https://raw.githubusercontent.com/yusefalhaj88-bit/Python/main/logo.png"

# --- الهندسة البصرية (إجبار السواد، الزجاج، وصورتك الخاصة) ---
st.markdown(f"""
    <style>
    /* إخفاء القوائم */
    header, footer, #MainMenu {{visibility: hidden;}}

    /* إجبار الخلفية على تحميل صورتك الفضائية */
    [data-testid="stAppViewContainer"] {{
        background: #000000 !important;
        background-image: linear-gradient(rgba(0,0,0,0.2), rgba(0,0,0,0.5)), url("{BG_URL}") !important;
        background-size: cover !important;
        background-position: center !important;
        background-attachment: fixed !important;
    }}

    /* تأثير الزجاج الشفاف للنوافذ */
    .glass-card {{
...
def run_ai(query):
    ...
    payload = {
        ...
        "messages": [{"role": "user", "content": f"اكتب وصف بيعي ذكي لمنتج: {query}"}]
    }
    try:
...
# --- الواجهة ---
# عرض اللوجو
st.markdown(f'<div style="text-align:center;"><img src="{LOGO_URL}" style="width:230px; filter:drop-shadow(0 0 20px #00f2ff);"></div>', unsafe_allow_html=True)
    ...
    if st.button("إطلاق الذكاء الشعاعي ✨"):
        if p_name:
            with st.spinner('ANALYZING...'):
                ...
                st.markdown(f"<div style='color:#00f2ff; background:rgba(0,0,0,0.8); padding:25px; border:1px solid #00f2ff; border-radius:20px; direction:rtl;'>{result}</div>", unsafe_allow_html=True)
